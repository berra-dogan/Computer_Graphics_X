...
class Scene {
    public:
        explicit Scene(std::vector<std::shared_ptr<Sphere>> spheres, std::vector<std::shared_ptr<LightSource>> lights, double ref_idx = 1.)
        ...
        std::pair<Intersection, int> findIntersection(const Ray& ray, int in_obj_idx = std::numeric_limits<double>::max()) const {
            ...
            return {closestIntersection, closestSphereIdx};  // Return the closest intersection (if any)
        }
        ...
        Vector getColorAux(std::default_random_engine &engine, const Vector P, const Vector N, int sphere_idx){
            ...
            for (const std::shared_ptr<LightSource> light_ptr: lights){
                ...
                std::pair<Intersection, int> shadowResult = findIntersection(shadowRay, sphere_idx);
                ...
                if (inShadow) {
                    ...
                    // Lo = Lo + ((light.I/(4*M_PI*d2)) * ( spheres[sphere_idx].albedo/M_PI) * std::max(0., dot(N, omega_i)));
                    // Lo = Lo + light.I/(4*M_PI*M_PI*light.R*light.R) * spheres[sphere_idx].albedo/M_PI /1000;
...
int main() {
    ...
    std::vector<std::shared_ptr<Sphere>> standard_scene_spheres = {
        ...
        std::make_shared<Sphere>(Vector(18, 0, 0), 10, Vector(0.8, 0.8, 0.8), false, true, 1.5, false), // transparent
        std::make_shared<Sphere>(Vector(0, 0, 0), 10, Vector(0, 0, 0), false, true, 1.5, false),       // hollow-transp 1
        std::make_shared<Sphere>(Vector(0, 0, 0), 10 - EPSILON, Vector(0, 0, 0), false, true, 1, false), // hollow-transp 2
